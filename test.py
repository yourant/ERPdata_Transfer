...
# 导出product表
# map表
data_map = pd.read_sql_query('select * from image_url_map', con=conn_pg_ol)
dic = {}
for i in range(data_map.shape[0]):
    dic[data_map.loc[i,'banma_url']] = data_map.loc[i,'aliyun_url']
for i in range(data_cp.shape[0]):
    if data_cp.loc[i,'SPU图片'] in data_map['banma_url'].values:
        data_cp.loc[i,'SPU图片'] = dic.get(data_cp.loc[i,'SPU图片'])
    if data_cp.loc[i,'SKU图'] in data_map['banma_url'].values:
        data_cp.loc[i,'SKU图'] = dic.get(data_cp.loc[i,'SKU图'])
    res = ''
    for sub_img in data_cp.loc[i, 'SPU图集'].split(','):
        if sub_img in data_map['banma_url'].values:
            res = res + dic.get(sub_img) + ','
        else:
            res = res + sub_img + ','
    res = res[:-1]
    data_cp.loc[i, 'SPU图集'] = res
data_cp.to_excel('/Users/edz/Documents/data_cppppp.xlsx')
...
for i in range(data_cp.shape[0]):
    ...
    if r is None:
        sql = '''INSERT INTO product (product_name,spu_code, primary_image, add_time, product_images, zebra_spu_id) VALUES ('{0}','{1}','{2}',now(),'{3}',{4})'''.format(
            data_cp.loc[i, '标题'].replace("'", "''"), data_cp.loc[i, 'SPU编码'], data_cp.loc[i, 'SPU图片'],
...
data_p_id = pd.read_sql_query('select * from product', con=conn_pg_ol)
...
data_product_attr = pd.read_sql_query('select * from product_attr', con=conn_pg_ol)
...
sup = pd.read_sql_query('select * from product', con=conn_pg_ol)
